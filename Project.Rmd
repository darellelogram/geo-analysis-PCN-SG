...
kde_sf_data<-function(sf){
  latlng<-st_coordinates(sf)
  sf_coords <- latlng[, c(2, 1)]
  #setDT(data_coords)
...
```
```{r}
amenities_list<-c("data\\exercisefacilities.json","data\\libraries.json","data\\preschools_location.json","data\\communityclubs.json")
kde_jsonlist_data(amenities_list)
```

##Skeleton code for filtering points per region
```{r}
subset<-subset(planning_area_sf, Name %in% c('PASIR RIS','MANDAI'))

sf_use_s2(FALSE)
st_intersects(mrt_exits_sf,subset)


within<-st_filter(mrt_exits_sf,subset)


tm_shape(subset(planning_area_sf, Name %in% c('PASIR RIS','MANDAI'))) + tm_text("Name", size=0.5) + tm_borders() + tmap_options(check.and.fix = TRUE) + tm_shape(within) + tm_symbols(size = 0.1, alpha = 0.5, col = "red") 
```
