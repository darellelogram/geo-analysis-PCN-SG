...
busstops_500buf_sp <- as_Spatial(busstops_500buf_sf, proj)
# access_points_sp <- as_Spatial(access_points_sf)
...
print(dim(access_points_sf)[1])
print(PCAPs_near_busstops / dim(access_points_sf)[1])s
```
#### DEBUG: try plotting
```{r}
tmap_mode("plot")
...
```{r}
near_bikeracks_count <- poly.counts(access_points_sp, bikeracks_500buf)
near_bikeracks_count / 306 * 100
```
```{r}
near_bikeracks_count
access_points_sp
```
